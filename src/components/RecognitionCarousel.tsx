...
export function RecognitionCarousel({ items }: { items: RecognitionItem[] }) {
    ...
    return (
      <div className="relative overflow-hidden rounded-[2.8rem] border border-[var(--ink)]/10 bg-white/52 p-8 shadow-[0_32px_110px_rgba(15,16,22,0.1)] backdrop-blur-xl">
        ...
        <p className="mt-4 max-w-2xl text-sm leading-7 text-[var(--ink)]/62">
          Certificates, media mentions and clinic milestones will appear here as they are approved.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="relative overflow-hidden rounded-[2.8rem] border border-[var(--ink)]/10 bg-white/52 p-4 shadow-[0_32px_110px_rgba(15,16,22,0.1)] backdrop-blur-xl">
      ...
      <div className="relative grid gap-5 lg:grid-cols-[0.42fr_1fr] lg:items-stretch">
        <div className="rounded-[2.1rem] border border-[var(--ink)]/10 bg-[var(--ink)] p-7 text-[var(--ivory)]">
          <BadgeCheck className="mb-12 h-7 w-7 text-[var(--champagne)]" />
            ...
            {visibleItems.map((item, index) => {
              ...
              return (
                <article
                  ...
                  className="relative min-w-0 flex-[0_0_82%] overflow-hidden rounded-[2.1rem] border border-white/66 bg-white shadow-[0_26px_90px_rgba(15,16,22,0.12)] sm:flex-[0_0_48%] lg:flex-[0_0_38%]"
                >
                  <div className="relative h-80 bg-[var(--mist)]">
                    {image ? (
                      <Image
                        ...
                        className={contain ? "object-contain p-7" : "object-cover"}
                        style={{
                  ...
                  <div className="p-6">
                    ...
                    <h4 className="font-serif text-3xl leading-none text-[var(--ink)]">
                      {item.title}
                    </h4>
                    {item.description ? (
...
function recognitionKey(item: RecognitionItem, index: number) {
  const image = item.image || item.badge;
  return image?.id || image?.contentHash || image?.src || `${item.type}-${item.title}-${index}`;
}
