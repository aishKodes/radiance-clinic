...
const submittedVideoWatchPageSlugs = new Set([
  "hair-transplant-aftercare-day-by-day",
...
async function build() {
  ...
  await writeCsv(
    ...
    hairVideoWatchPages.map((page) => ({
      ...
      page_indexed: submittedVideoWatchPageSlugs.has(page.slug)
        ? "URL is not on Google at 2026-09-24 inspection"
        : "PENDING_URL_INSPECTION",
      indexing_request_status: submittedVideoWatchPageSlugs.has(page.slug)
        ? "Accepted in Google's priority crawl queue on 2026-09-24"
        : "Not submitted; await URL Inspection",
      video_detected: "PENDING_VIDEO_INDEXING_REPORT",
      video_indexed: "PENDING_VIDEO_INDEXING_REPORT",
      reason_or_next_action: submittedVideoWatchPageSlugs.has(page.slug)
        ? "Wait for Google to crawl and reassess this page. Do not infer video index status from an embedded player."
        : "Live in the public video sitemap; inspect selected P0 pages in Search Console. Do not infer video index status from an embedded player.",
